  ...
  this.receiveListener = function(result) {
    ...
    if(self.bytesRead > 0 && self.sizeOfMessage > 0) {      
      ...
      if(remainingBytes > result.length) {
        ...
        var buffer = new Buffer(self.buffer.length + result.length);
        self.buffer.copy(buffer, 0, 0, self.buffer.length);
        result.copy(buffer, self.buffer.length, 0, result.length);
        ...
        var buffer = new Buffer(self.buffer.length + remainingBytes);
        self.buffer.copy(buffer, 0, 0, self.buffer.length);
        result.copy(buffer, self.buffer.length, 0, remainingBytes);
        // Emit the message
        self.emit("data", buffer.toString('binary', 0, buffer.length));
        // Reset the variables
        self.buffer = new Buffer(0); self.bytesRead = 0; self.sizeOfMessage = 0;
        // If message is longer than the current one, keep parsing
        if(remainingBytes < result.length) {
          self.receiveListener(result.slice(remainingBytes, result.length));
        }
      }
    } else {
      if(self.stubBuffer.length > 0) {
        // Add missing stub files
        var buffer = new Buffer(self.stubBuffer.length + result.length);
        self.stubBuffer.copy(buffer, 0, 0, self.stubBuffer.length);
        result.copy(buffer, self.stubBuffer.length, 0, result.length);
        // New result contains the former missing bytes as well as the incoming payload
        result = buffer;
        self.stubBuffer = new Buffer(0);
      }
      ...
      if(result.length > 4) {
        ...
        if(sizeOfMessage > result.length) {
          ...
          self.buffer = buffer;
          // Adjust variables
          self.bytesRead = result.length; self.sizeOfMessage = sizeOfMessage;
        } else if(sizeOfMessage == result.length) {
          self.emit("data", result.toString('binary', 0, result.length));
        } else if(sizeOfMessage < result.length) {
          self.emit("data", result.slice(0, sizeOfMessage).toString('binary', 0, sizeOfMessage));
          self.receiveListener(result.slice(sizeOfMessage, result.length));
        }
      } else {
...
Connection.prototype.send = function(command) {
  ...
  try {
    // var command_string = command.toBinary();
    // var buffer = new Buffer(command_string.length);
    // buffer.write(command_string, 'binary', 0);
      ...
      if(this.connection.currently_reconnecting == null) {
        ...
        new_connection.addListener("connect", function() {
          ...
          while(self.messages.length > 0) {
            // var command_string = self.messages.shift().toBinary();
            // var buffer = new Buffer(command_string.length);
            // buffer.write(command_string, 'binary', 0);
            // this.connection.write(buffer);
