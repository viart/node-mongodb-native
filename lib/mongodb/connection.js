...
var ReplicaSets = exports.ReplicaSets = function(servers) {
    ...
    if(servers.constructor != Array || !servers.length) {
        throw Error("The parameter must be an array of servers");
    } else {
        var serversCorrected = servers.every(function(server) {
            return server instanceof Server;
        });
        if (!serversCorrected) throw Error("All server entries must be of type Server");
    ...
    this.__defineGetter__("masterConnection", function() {
        ...
        if (!this.master || !this.master.master) {
            ...
            if (!this.master) throw new Error("Master server not found")
        }

        return this.master ? this.master.masterConnection : null;
    });
};

//TODO: add existens check
ReplicaSets.prototype.pushServer = function(server) {
