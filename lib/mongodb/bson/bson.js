...
BSON.deserialize = function(data, is_array_item, returnData, returnArray) {
  ...
  var size = BinaryParser.toInt(data.slice(0, 4).toString('binary', 0, 4));  
  // Adjust index
  index = index + 4;
  
  while(index < data.length) {
    // Read the first byte indicating the type of object
    var type = BinaryParser.toSmall(data.slice(index, index + 1).toString('binary', 0, 1));
    var insert_index = 0;
    ...
    if(type == BSON.BSON_DATA_STRING) {
      ...
      var string_name = data.slice(index, string_end_index).toString('binary', 0, (string_end_index - index));
      ...
      var string_size = BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4));
      // Adjust the index to point at start of string
      index = index + 4;
      // Read the string
      var value = data.slice(index, index + string_size - 1).toString('binary', 0, string_size - 1);
      ...
      var value = BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4));
      ...
      // Read the number value
      var low_bits = Integer.fromInt(BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4)));
      var high_bits = Integer.fromInt(BinaryParser.toInt(data.slice(index + 4, index + 4 + 4).toString('binary', 0, 4)));
      // Create to integers
      ...
      // Read the number value
      var value = BinaryParser.toDouble(data.slice(index, index + 8).toString('binary', 0, 8));
      // Adjust the index with the size
      index = index + 8;
      // Set the data on the object
      is_array_item ? return_array[insert_index] = value : return_data[string_name] = value;
      ...
      var object_size = BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4));
      ...
      // Read the size of the object
      var array_size = BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4));
      ...
      // Read the length of the string (next 4 bytes)
      var boolean_value = BinaryParser.toSmall(data.slice(index, index + 1).toString('binary', 0, 1));
      ...
      var oid = data.slice(index, index + 12).toString('binary', 0, 12);
      // Calculate date with miliseconds
      ...
      var total_code_size = BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4));
      index = index + 4;
      var string_size = BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4));
      index = index + 4;
      // Read the string + terminating null
      var code_string = BinaryParser.decode_utf8(data.slice(index, index + string_size - 1).toString('binary', 0, string_size - 1));
      index = index + string_size;
      // Get the bson object
      ...
      var total_number_of_bytes = BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4));
      index = index + 4;
      // Decode the subtype
      var sub_type = BinaryParser.toByte(data.slice(index, index + 1).toString('binary', 0, 1));
      index = index + 1;
      // Read the size of the binary
      var number_of_bytes = BinaryParser.toInt(data.slice(index, index + 4).toString('binary', 0, 4));
      index = index + 4;
      // Read the next bytes into our Binary object
      var bin_data = data.slice(index, index + number_of_bytes).toString('binary', 0, number_of_bytes);
      // Binary object
      var value = new exports.Binary(bin_data);
