            ...
            if(err != null) {
                ...
                if (info.ismaster == 1) {
                    ...
                    if (self.masterRequested) {
                        ...
                        var isMasterInList = self.serverConfig.servers.some(function(server){
                            return (server.host == masterInfo[0] && server.port == masterInfo[1]);
                        });

                        if (!isMasterInList) {
                            var masterServer = new Server(masterInfo[0], masterInfo[1]);
                            // add to seed list
                            self.serverConfig.pushServer(masterServer);
                            self.initServer(masterServer, callback);
                        } else {
                            callback(null, null);
                        }
                    }
...
Db.prototype.open = function(callback) {
  ...
  // Set up connections
  if (self.serverConfig instanceof ReplicaSets) {
      self.options.replicaSet = true;
      self.rsGetMaster(callback)
      return;
  }
  ...
  if(self.serverConfig instanceof Server) {
    ...
    self.serverConfig.connection.addListener("connect", function() {
      ...
      var connectCallback = function(err, reply) {        
        ...
        if(self.state == 'notConnected' || self.state == 'connectiong') {
          self.state = 'connected';
          self.emit('connected');
          callback(null, self);
        } else {
      ...
      server.connection.addListener("connect", function() {
        ...
        var connectCallback = function(err, reply) {
          if(err != null) {
            ...
            // emit a message saying we got a master and are ready to go and change state to reflect it
            if(++numberOfConnectedServers == serverConnections.length && (self.state == 'notConnected' || self.state == 'connecting')) {
              self.state = 'connected';
              self.emit('connected');
              callback(null, self);
            }            
            ...
            var col = new function()
            {
                ...
                methods.forEach(function(fn){
                    col[fn] = function(){
                        ...
                        else if (db.state == 'connecting' || db.state == 'masterSearch') {
  ...
  if (this.state == 'connected') {
      try{
          ...
          if (this.options.replicaSet) {
              // mark master server as non-master
              this.serverConfig.master.master = false;
              this.state = 'notConnected';
