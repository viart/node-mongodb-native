...
var Db = exports.Db = function(databaseName, serverConfig, options) {
  ...
  this.bson_serializer = this.options.native_parser ? require('bson') : require('./bson/bson');
  this.bson_deserializer = this.options.native_parser ? require('bson') : require('./bson/bson');  
  this.connections = [];
  this.commandsQueue = [];
  // State of the db connection
  this.state = 'notConnected';
  this.masterRequested = false;
...
Db.prototype.initServer = function(server, callback) {
    var self = this;
    ...
    server.connection.addListener("connect", function() {
        var connectCallback = function(err, reply) {
            if(err != null) {
                ...
                var info = reply.documents[0];
                if (info.ismaster == 1) {

                    // push master to connections (to handle close on DB object)
                    self.connections.push(server.connection);

                    // set master connection as current connection
                    ...
                    // close current (not-master) connection
                    server.close();

                    // try direct init master
                    if (self.masterRequested) {
                        callback(null, null);
                    } else {

                        // prevent concurency
                        self.masterRequested = true;

                        var masterInfo = info.primary.split(':');

                        // is master in seed servers list?
                        var isMasterInList = false;
                        self.serverConfig.servers.forEach(function(server){
                            if (server.host == masterInfo[0] && server.port == masterInfo[1]) {
                                isMasterInList = true;
                                return false;
                            }
                        });
                        ...
                        if (!isMasterInList) {
                            self.initServer(new Server(masterInfo[0], masterInfo[1]), callback);
                        } else {
                            callback(null, null);
                        }
                    }
...
// fire on send error (if we use Replica Sets)
//TODO: refactor to EventEmiter on Error event
Db.prototype.masterDown = function(command) {
    var self = this;

    //10 sec are default replica heartbeat timeout
    //TODO: get heartbeatTimeout from replica info
    var timeout = self.options.heartbeatTimeout || 10000;

    // add to queue broken commands
    if (command) {
        self.commandsQueue.push(command);
    }

    if (self.state != "masterSearch") {
        self.state = "masterSearch";

        // use timeout because Replica Sets need some time to understand that master server is down
        //TODO: mb, depends of command type (find, count, etc...) use Slave server to get data? (see: rs.slaveOk())
        setTimeout(function(){
            self.rsGetMaster(function(err, db){
                if(db) {
                    // execute broken commands
                    self.commandsQueue.forEach(function(command){
...
Db.prototype.rsGetMaster = function(callback) {

    var self = this,
        servers = this.serverConfig.servers,
        iterations = servers.length,
        res = null;

    servers.forEach(function(server) {
        self.initServer(server, function(err, data){
...
Db.prototype.open = function(callback) {
  ...
  //TODO: add detection by serverConfig type (like ServerPairs or ServerCluster)
  if (this.options.replicaSet) {
      this.rsGetMaster(callback)
      return;
  }
  ...
  if(self.serverConfig instanceof Server) {
    ...
    serverConnections.forEach(function(server) {
      ...
      server.connection.addListener("error", function(err) {
        if(self.listeners("error") != null && self.listeners("error").length > 0) self.state = "notConnected"
        return callback(err, null);
      });
...
Db.prototype.initCollections = function() {
    // Closure
    var db = this;
    // Define the list of dynamic methods for collection
    var methods = ['count', 'find', 'findOne', 'insert', 'update', 'remove'];
    var connecting = false;
    // For each 'collection name'
    for (var i = 0; i < arguments.length; i++) {
        (function(name){
            // Create magic collection
            var col = new function()
            {
                require('sys').puts('Define magic collection ' + name);
                var col = this;
                this.name = name;
                // Link to original collection object
                this.collection = null;
                // Store all calls to this collection in temporary buffer
                // while db is connecting and collection is not selected yet
                this.buffer = [];

                // Flush the buffer to original collection
                this.flush = function()
                {
                    if (!col.buffer.length) {
                        return;
                    }
                    require('sys').puts('Flush buffer of ' + col.name + ' (' + col.buffer.length + ')');
                    while (col.buffer.length) {
                        var call = col.buffer.shift();
                        col.collection[call[0]].apply(col.collection, call[1]);
                    }
                }

                this.selectCollection = function(callback)
                {
                    // We store all collections in master connection object
                    var conn = db.serverConfig.masterConnection;
                    if (typeof(conn.collections) == 'undefined') {
                        conn.collections = {};
                    }
                    // Check if collection in not initialized
                    if (conn.collections[name]) {
                        // Flush the buffer
                        col.flush();
                    }
                    else {
                        require('sys').puts('Select collection ' + name);
                        db.collection(col.name, function(err, collection){
                            require('sys').puts('collection selected: ' + name);
                            if (err) {
                ...
                {
                    require('sys').puts('Connecting to db');
                    connecting = true;
                    db.open(function(err, res){
                        ...
                        require('sys').puts('db connected');
                        db.emit('connected');
                        col.selectCollection(callback);
                    })
                }

                // Create dynamic methods
                methods.forEach(function(fn){
                    require('sys').puts('Define magic method ' + name + '.' + fn + '()');
                    col[fn] = function(){
                        require('sys').puts('Buffer call to ' + name + '.' + fn + '()');
                        // Add this call to buffer
                        col.buffer.push([fn, arguments]);
                        // Find callback in arguments passed to this function
                        var callback = null;
                        if (arguments.length) {
                        ...
                        if (db.state == 'connected') {
                            ...
                            // initialize collection when db will be connected
                            ...
                            // connect to db
                            col.connect(callback);
                        }
                    }
                })
            }

            // Create dynamic getter for this collection (e.g. db.myCollection)
            db.__defineGetter__(name, function(){
                // This getter returns an object which will proxy all methods for desired collection
                return col;
            })
